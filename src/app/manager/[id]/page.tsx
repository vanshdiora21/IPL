...
// Crex CDN for player photos (directly from their akamaized CDN — no scraping needed)
...
export default function ManagerPage({ params }: { params: { id: string } }) {
  ...
  const enriched = manager.players.map(p => ({
    ...p,
    points: getPlayerPts(p.name),
    isCap: manager.captain === p.name,
    isVC: manager.viceCaptain === p.name,
    isSuper: manager.super3xPlayer === p.name,
  })).map(p => ({
    ...p,
    multiplier: p.isCap ? 2 : p.isVC ? 1.5 : p.isSuper ? 3 : 1,
    effectivePts: p.points * (p.isCap ? 2 : p.isVC ? 1.5 : p.isSuper ? 3 : 1),
  }));
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.hero}>
        ...
        <div className={styles.heroStats}>
          {[
            ...
            { val: manager.players.length, lbl:'Players', gold:false },
        ...
        <div className={styles.sorts}>
          {(['pts','role','price'] as const).map(s => (
            <button key={s} onClick={() => setSort(s)}
              className={`${styles.sBtn} ${sort===s ? styles.sActive : ''}`}>
        ...
        {filtered.map((p, i) => {
          ...
          return (
            <div key={p.name} className={styles.card} style={{ animationDelay:`${i*.04}s` }}>
              ...
              {/* Player image from Crex CDN */}
              <div className={styles.imgWrap}>
                {imgOk ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={CREX_IMG(p.crexId)}
                    alt={p.shortName}
                  ...
                  <div className={styles.fallbackAvatar} style={{ background: tc.bg }}>
                    {p.shortName.replace('. ', '')[0]}{p.shortName.split(' ').pop()![0]}
                  </div>
                )}
              ...
              <div className={styles.info}>
                ...
                <div className={styles.pMeta}>
                  <div className={styles.pTeam} style={{ background: tc.bg, color: tc.text }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={CREX_TEAM_IMG(p.team)} alt={p.team} width={14} height={14} style={{borderRadius:2,flexShrink:0}} onError={e => (e.currentTarget.style.display='none')} />
                    {p.team}
                  </div>
      ...
      <div className={styles.breakdown}>
        ...
        {ROLE_ORDER.map(role => {
          const rPts = byRole[role]?.reduce((s, p) => s + p.effectivePts, 0) || 0;
          ...
          return (
            <div key={role} className={styles.barRow}>
              ...
              <div className={styles.barTrack}>
                <div className={styles.barFill} style={{ width:`${pct}%`, background: colors[role], transformOrigin:'left', animation:'grow 1s ease both' }} />
              </div>
      ...
      <div className={styles.footer}>
        ...
        <div className={styles.crexNote}>
          Player photos & stats via <a href="https://crex.com" target="_blank" rel="noopener" style={{color:'var(--orange)'}}>CREX</a>
        </div>
      </div>
    </div>
  );
